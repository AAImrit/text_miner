...
def test_expression(regex_expression, file_path, content):
    '''
...
def get_closure_info (common_issues_path, common_issue_list, user_search, re_expression):
    ...
    new_issue = {'name': name_issue, 'looking':user_search, 'regex':re_expression}
...
def create_new_issue_main (content, file_path, common_issues_path):
    user_validation = "1"
    ...
    while user_validation != "0":
        ...
        test_expression(expression, file_path, content)
        ...
        if user_validation == "0":
            ...
            test_expression(expression, file_path[:-8]+name+".csv", content)
